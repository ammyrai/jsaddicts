...
if ( ! class_exists( 'Ultra_Seven_Info' ) ) {
	...
	class Ultra_Seven_Info {
		 ...
		 * Single instance.
		 *
		 * @var Ultra_Seven_Info $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Ultra_Seven_Info ) ) {
				self::$instance = new Ultra_Seven_Info;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s info', 'ultra-seven' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s info', 'ultra-seven' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-details';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme details page.', 'ultra-seven' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'ultra-seven' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'ultra_seven_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'ultra-seven' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_ultra_seven_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_ultra_seven_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'ultra-seven' ), $method );
			}
		}
		...
		public function useful_plugins() {
			...
			<div class="wpopsection wpopsection-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'ultra-seven' ); ?></a>
				<?php endif; ?>
            ...
            <table class="table">
                <thead>
                    <tr>
                            ...
                            <th class="db-bk-color-one"><?php _e('Product Features', 'ultra-seven'); ?></th>
                            <th class="db-bk-color-two"><?php _e('Free', 'ultra-seven'); ?></th>
                            <th class="db-bk-color-two"><?php _e('Premium', 'ultra-seven'); ?></th>
                    </tr>
                </thead>
                 <tbody>
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'ultra-seven-info', get_template_directory_uri() . '/inc/welcome/css/info.css', array(), '1.0.0' );
				wp_enqueue_script( 'ultra-seven-info', get_template_directory_uri() . '/inc/welcome/js/info' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'ultra_seven_dismiss_status', true );
			?>
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'ultra_seven_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'ultra_seven_dismiss_status', 1 );
